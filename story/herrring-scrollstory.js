...
function boot(root, host, opt) {
  ...
  function start() {
    ...
    [80, 400, 1200].forEach(t => setTimeout(() => {
      H = fitHeight(host, opt.scrollHeight);
      if (window.ScrollTrigger) window.ScrollTrigger.refresh();
    }, t));
    let lastVH = innerHeight;
    addEventListener("resize", () => {
      if (Math.abs(innerHeight - lastVH) < 60) return;   // Adressleiste ignorieren
      lastVH = innerHeight;
      H = fitHeight(host, opt.scrollHeight);
      if (window.ScrollTrigger) window.ScrollTrigger.refresh();
    }, {passive:true});
    setTimeout(() => {
      const r = room();
      console.log("[scrollstory] " + VERSION +
        " — Strecke " + Math.round(H - innerHeight) + "px" +
        (r == null ? "" : ", Platz bis zum nächsten Abschnitt " + r + "px"));
      if (r != null && r < H - 20) {
        console.warn("[scrollstory] Die Sektion ist zu kurz: " + r + "px vorhanden, " +
          H + "px nötig. Entweder die Sektion im Wix-Editor auf mindestens " + H +
          "px ziehen — oder scroll-height auf " + Math.round(r/innerHeight*100) +
          "vh setzen, dann läuft die Animation vollständig im vorhandenen Platz ab.");
      }
    }, 1400);
